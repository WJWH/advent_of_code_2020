...
neighborsCoords (x,y,z,w) = filter (/= (x,y,z,w)) $ (,,,) <$> [x-1..x+1] <*> [y-1..y+1] <*> [z-1..z+1] <*> [w-1..w+1]

singleCube :: PocketDimension -> Position -> (Position,Cube)
singleCube pd pos@(x,y,z,w) = case cube of
  ...
  where numActiveNeighbors = sum [ 1 | nc <- neighborsCoords (x,y,z,w) , M.lookup nc pd == Just Active]
        cube = fromMaybe Inactive $ M.lookup (x,y,z,w) pd
...
part2 = do
  ...
  let initialCoords = [ (x,y,0,0) | y <- [1..lineWidth], x <- [1..lineHeight]] 
